    ...
    public class RegisterCHandler : IRequestHandler<RegisterCommand, RegisterResponse>
    {
        ...
        public async Task<RegisterResponse> Handle(RegisterCommand command, CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("User with UserName {UserName} already exists", user.UserName.Value);
                    throw new BusinessRuleException<UserField>(
                ...
                {
                    _logger.LogWarning("User with Email {Email} already exists", user.Email.Value);
                    throw new BusinessRuleException<UserField>(
                        ErrorCategory.Conflict,
                        UserField.Email,
                ...
                {
                    _logger.LogWarning("User with Phone {Phone} already exists", user.Phone.Value);
                    throw new BusinessRuleException<UserField>(
                        ErrorCategory.Conflict,
                        UserField.Phone,
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex,
                    "Failed to register User. Request: {@Request}",
                    command.Request
                );
                throw;
            }
