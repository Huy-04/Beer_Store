    ...
    public class CreatePermissionCHandler : IRequestHandler<CreatePermissionCommand, PermissionResponse>
    {
        ...
        public async Task<PermissionResponse> Handle(CreatePermissionCommand command, CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("Create failed: Permission with Name '{Name}' already exists", permission.PermissionName.Value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex,
                    "Exception occurred while creating Permission. Request: {@Request}",
                    command.Request
                );
                throw;
            }
