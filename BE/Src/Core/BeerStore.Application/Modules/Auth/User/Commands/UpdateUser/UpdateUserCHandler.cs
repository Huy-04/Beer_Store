    ...
    public class UpdateUserCHandler : IRequestHandler<UpdateUserCommand, UserResponse>
    {
        ...
        public async Task<UserResponse> Handle(UpdateUserCommand command, CancellationToken token)
        {
            ...
            try
            {
                ...
                if (user == null)
                {
                    _logger.LogWarning("Update failed: User with Id={Id} not found", command.IdUser);
                ...
                {
                    _logger.LogWarning("Update failed: User with UserName '{UserName}' already exists", newUserName.Value);
                    throw new BusinessRuleException<UserField>(
                ...
                {
                    _logger.LogWarning("Update failed: User with Email '{Email}' already exists", newEmail.Value);
                    throw new BusinessRuleException<UserField>(
                        ErrorCategory.Conflict,
                        UserField.Email,
                ...
                {
                    _logger.LogWarning("Update failed: User with Phone '{Phone}' already exists", newPhone.Value);
                    throw new BusinessRuleException<UserField>(
                        ErrorCategory.Conflict,
                        UserField.Phone,
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex,
                    "Exception occurred while updating User. Request: {@Request}",
                    command.Request
                );
                throw;
            }
