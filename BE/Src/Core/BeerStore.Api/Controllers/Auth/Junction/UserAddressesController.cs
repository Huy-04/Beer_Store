...
using BeerStore.Application.Modules.Auth.Junction.UserAddresses.Commands.CreateUserAddress;
using BeerStore.Application.Modules.Auth.Junction.UserAddresses.Commands.RemoveUserAddress;
using BeerStore.Application.Modules.Auth.Junction.UserAddresses.Commands.UpdateUserAddress;
using BeerStore.Application.Modules.Auth.Junction.UserAddresses.Queries.GetAllUserAddress;
using BeerStore.Application.Modules.Auth.Junction.UserAddresses.Queries.GetUserAddressById;
using BeerStore.Application.Modules.Auth.Junction.UserAddresses.Queries.GetUserAddressByPhone;
...
namespace BeerStore.Api.Controllers.Auth.Junction
{
    ...
    public class UserAddressesController : BaseApiController
    {
        ...
        {
            var result = await _mediator.Send(new GetAllUserAddressQuery(CurrentUserId), token);
            return Ok(result);
        }
        ...
        {
            await _mediator.Send(new RemoveUserAddressCommand(id, CurrentUserId), token);
            return NoContent();
        }
    }
}
